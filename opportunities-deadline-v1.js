(() => {
  ...
  oppsPage=function(){
    ...
    const header=`<div class="opp-list-head" style="display:grid;grid-template-columns:minmax(0,1fr) 150px 24px;gap:16px;align-items:center;padding:0 18px 8px;color:#6c7a73;font-size:12px;font-weight:700;text-transform:uppercase;letter-spacing:.04em"><span>Oportunidad</span><span>Deadline</span><span></span></div>`;
    const rows=list.map(o=>`<a class="app-row" href="#opportunity/${o.id}" style="display:grid;grid-template-columns:minmax(0,1fr) 150px 24px;gap:16px;align-items:center"><div><strong>${esc(o.title)}</strong><span>${esc(o.country)}</span></div><div class="opp-deadline" style="font-weight:700;white-space:nowrap">${esc(deadlineLabel(o))}</div><b>›</b></a>`).join('');
