(() => {
  ...
  const actionButtons=d=>isUsable(d)
  ...
  openReq=function(id,rid){
    ...
    const candidates=state.documents.filter(d=>isUsable(d)&&matches(d,q));
    const best=candidates[0];
    const existing=best?`<div class="existing-doc-label">Versión utilizable en 2026</div><div class="existing-doc-card truthful"><div class="doc-list-main"><span class="real-file-mark">${mark(best)}</span><span><b>${esc(best.name)}</b><small>${esc(label(best))}</small></span></div>${actionButtons(best)}</div>`:`<div class="empty compact-empty"><b>Todavía no lo tenemos</b><span>No hay una versión utilizable en 2026 guardada.</span></div>`;
  ...
  function bindViewerButtons(){
    document.querySelectorAll('[data-view-doc]').forEach(el=>{el.onclick=e=>{e.preventDefault();e.stopPropagation();const d=state.documents.find(x=>x.id===el.dataset.viewDoc);if(isUsable(d))window.postulaDocuments?.visualize(d).catch(console.error)}});
