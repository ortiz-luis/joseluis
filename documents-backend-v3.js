(() => {
  ...
  async function renderPdf(blob,container){
    ...
    for(let n=1;n<=pdf.numPages;n++){
      ...
      const scale=Math.min(1.8,Math.max(0.6,Math.max(320,(container.clientWidth||900)-24)/base.width));
      const viewport=page.getViewport({scale}),wrap=document.createElement('div'),canvas=document.createElement('canvas'),dpr=Math.min(window.devicePixelRatio||1,2);
      wrap.style.cssText='margin:0 auto 16px;background:#fff;width:max-content;max-width:100%;box-shadow:0 1px 8px rgba(0,0,0,.10)';
      canvas.width=Math.floor(viewport.width*dpr);canvas.height=Math.floor(viewport.height*dpr);canvas.style.width=`${viewport.width}px`;canvas.style.height=`${viewport.height}px`;canvas.style.maxWidth='100%';canvas.style.display='block';
  ...
  async function visualize(doc){
    ...
    const isPdf=/pdf/i.test(blob.type)||/\.pdf$/i.test(doc.storagePath||doc.name||'');
    const isImage=/^image\//i.test(blob.type)||/\.(png|jpe?g|webp)$/i.test(doc.storagePath||doc.name||'');
    ...
    const body=isPdf?`<div class="viewer-frame" style="overflow:auto;background:#eef2f0;padding:12px"><div id="pdfjs-v3"><div class="viewer-empty"><b>Cargando PDF…</b></div></div></div>`:isImage?`<div class="viewer-frame"><img id="postula-image-v3" alt="${esc(doc.name)}" style="max-width:100%;max-height:75vh;object-fit:contain"></div>`:`<div class="viewer-frame"><div class="viewer-empty"><b>Vista previa no disponible</b><span>Puedes descargar el original.</span></div></div>`;
    openModal(`<div class="viewer-shell"><div class="viewer-toolbar"><strong>${esc(doc.name)}</strong><div class="viewer-actions"><button type="button" class="button soft" id="db-download-v3">Descargar</button></div></div>${body}</div>`);
    const dl=document.querySelector('#db-download-v3');if(dl)dl.onclick=()=>download(doc,blob);
    if(isPdf){const target=document.querySelector('#pdfjs-v3');try{await renderPdf(blob,target)}catch(err){console.error('PDF.js v3 failed',err);if(target)target.innerHTML='<div class="viewer-empty"><b>No se pudo renderizar el PDF</b><span>Puedes descargar el original.</span></div>';}}
    else if(isImage){const u=URL.createObjectURL(blob),img=document.querySelector('#postula-image-v3');if(img)img.src=u;modal?.addEventListener('close',()=>URL.revokeObjectURL(u),{once:true});}
    return true;
  }
