(() => {
  ...
  const candidateRows=()=>{
    if(!people)return '';
    if(!(people.candidates||[]).length)return '<div class="account-muted">Cuando Joseluis cree su cuenta, aparecerá aquí para darle acceso.</div>';
    return people.candidates.map(p=>`<div class="account-member"><div><strong>${esc(p.display_name||p.email||'Nuevo usuario')}</strong><span>${esc(p.email||'')}</span></div><button type="button" class="button soft" data-workspace-add="${esc(p.user_id)}">Dar acceso</button></div>`).join('');
  };

  profilePage=function(){
    const u=session()?.user,i=info(),email=u?.email||'Cargando…';
    const admin=i?.isAdmin?`<section class="account-section"><div class="account-section-head"><div><h2>Acceso al espacio</h2><p>${esc(i.workspaceName||'Postulaciones Joseluis')}</p></div></div><div class="account-member-list">${memberRows()}</div><div class="account-candidates"><h3>Añadir usuario</h3>${candidateRows()}</div></section>`:'';
    return `<section class="page account-page"><div class="page-head compact"><h1>Cuenta</h1></div><div class="profile-app account-app"><div><span>Email</span><strong>${esc(email)}</strong></div><div><span>Rol</span><strong>${roleLabel(i)}</strong></div><div><span>Espacio</span><strong>${esc(i?.workspaceName||'Sin acceso asignado')}</strong></div><div><span>Documentos</span><strong>Privados y compartidos sólo dentro del espacio autorizado</strong></div></div>${admin}</section>`;
  };

  const previousBind=bind;
  bind=function(){
    previousBind();
    document.querySelectorAll('[data-workspace-add]').forEach(btn=>btn.onclick=async e=>{e.preventDefault();btn.disabled=true;try{people=await window.postulaWorkspace.addMember(btn.dataset.workspaceAdd);render()}catch(err){alert(err.message);btn.disabled=false}});
