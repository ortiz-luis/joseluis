(() => {
  ...
  const roleLabel=d=>window.postulaDocumentRoleLabel?.(d?.documentRole)||'';
  ...
  const nested=d=>`<div class="doc-item-shell"><div class="doc-item"><div class="doc-list-main"><span class="real-file-mark">${mark(d)}</span><div><strong>${esc(d.name)}</strong><span>${roleLabel(d)?`<b class="doc-role-tag">${esc(roleLabel(d))}</b>`:'Listo'}</span></div></div>${actions(d)}</div><button type="button" class="doc-trash" data-delete-doc="${esc(d.id)}" aria-label="Eliminar ${esc(d.name)}" title="Eliminar documento">${trashIcon()}</button></div>`;
  ...
  document.addEventListener('click',e=>{
    ...
    window.postulaDocuments?.clearUploadContext?.();
