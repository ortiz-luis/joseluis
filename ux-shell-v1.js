(()=>{
 ...
 const labels={home:'Inicio',opportunities:'Oportunidades',opportunity:'Oportunidad',documents:'Documentos',create:'CV y cartas',templates:'Plantillas',profile:'Perfil',settings:'Configuración',admin:'Administración'};
 ...
 function signedInName(){
   const info=window.postulaWorkspace?.info?.();
   const session=window.postulaAuth?.getSession?.();
   const profile=info?.profile||{};
   const meta=session?.user?.user_metadata||{};
   return String(profile.display_name||meta.display_name||meta.full_name||'').trim();
 }
 ...
 function renderAccount(){
   ...
   foot.innerHTML=`<a class="ux-account" href="#profile"><span>${esc(name||'Cuenta')}</span></a><button class="ux-signout" type="button" data-ux-signout>Cerrar sesión</button>`;
   foot.querySelector('[data-ux-signout]')?.addEventListener('click',async e=>{e.preventDefault();e.stopPropagation();const btn=e.currentTarget;btn.disabled=true;btn.textContent='Saliendo…';try{await window.postulaAuth?.signOut?.()}catch{btn.disabled=false;btn.textContent='Cerrar sesión'}});
 }
 ...
 function build(){
  ...
  renderRecents();renderAccount();refreshTitle();syncAria();
 }
 function onRoute(){track();renderRecents();renderAccount();refreshTitle();renderResults();if(mobile())collapse()}
 window.addEventListener('hashchange',onRoute);
 window.addEventListener('postula-workspace-ready',()=>{renderAccount();refreshTitle()});
